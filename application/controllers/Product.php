...
class Product extends CI_Controller
{
    ...
    public function stockintegrasi_json()
    {
        ...
        foreach ($list->result() as $stock) {
            ...
            // Check if jenis_transaksi exists in badge_colors array
            ...
            $row = array(
                ...
                '<span class="badge bg-' . $badge_colors[$stock->jenis_transaksi] . '">' . ucwords(str_replace('_', ' ', $stock->jenis_transaksi)) . '</span>',
                $stock->stock_sebelum,
                $stock->stock_sesudah,
            );
            $data[] = $row;
    ...
    public function json_opname()
    {
        ...
        $list = $this->Datatable_model->get_data('tb_opname', $columns, $joins, $filter, $this->input->post('search')['value'], $where);
    ...
    public function json_temp_fisik()
    {
        ...
        $list = $this->Datatable_model->get_data('tb_opname_fisik', $columns, $joins, $filter, $this->input->post('search')['value'], $where);
    ...
    public function json_temp_rusak()
    {
        ...
        $list = $this->Datatable_model->get_data('tb_opname_rusak', $columns, $joins, $filter, $this->input->post('search')['value'], $where);
    ...
    public function simpan_fisik($id)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $update = $this->M_Crud->update_data(['id_brg' => $value['id_brg']], ['stock_brg' => $value['stock_fisik']], 'tb_barang');
            $riwayat = [
                'id_brg' => $value['id_brg'],
                'tgl_riwayat_stock' => date('Y-m-d H:i:s'),
                'jumlah_riwayat' => $value['stock_fisik'] - $barang['stock_brg'],
    ...
    public function simpan_spoil($id)
    {
        ...
        foreach ($data as $key => $value) {
            // INPUT RIWAYAT
            $barang = $this->M_Crud->show('tb_barang', ['id_brg' => $value['id_brg']])->row_array();
            $update = $this->M_Crud->update_data(['id_brg' => $value['id_brg']], ['stock_brg' => $barang['stock_brg'] - $value['jumlah_rusak']], 'tb_barang');
            $riwayat = [
                'id_brg' => $value['id_brg'],
                'tgl_riwayat_stock' => date('Y-m-d H:i:s'),
                'jumlah_riwayat' => $value['jumlah_rusak'],
                'jenis_transaksi' => 'stock_rusak',
                'stock_sebelum' => $barang['stock_brg'],
                'stock_sesudah' => $barang['stock_brg'] - $value['jumlah_rusak'],
        ...
        if (count($sheet) <= 1) {
            ...
            foreach ($sheet as $x => $excel) {
                ...
                $riwayat = [
                    'id_brg' => $id,
                    'tgl_riwayat_stock' => date('Y-m-d H:i:s'),
                    'jumlah_riwayat' => $excel["9"],
                    'jenis_transaksi' => 'stock_awal',
                    'stock_sebelum' => 0,
                    'stock_sesudah' => $excel["9"],
    ...
    public function json_temp_ubah($id)
    {
        ...
        $list = $this->Datatable_model->get_data('tb_pemesanan_detail', $columns, $joins, $filter, $this->input->post('search')['value'], $where);
    ...
    public function json_pemesanan()
    {
        ...
        foreach ($list->result() as $pemesanan) {
            ...
            $row = array(
                $no,
                date('d-m-Y H:i', strtotime($pemesanan->tgl_pesan)),
